...
export  const processBpsDynamicDataContent = (bpsDynamicData: TBpsDynamicData) => {
            ...
            for (let turvarCount = 0; turvarCount < turvar.length; turvarCount++) {
                for (let thCount = 0; thCount < tahun.length; thCount++) {
                    for (let turthCount = 0; turthCount < turtahun.length; turthCount++) {
                        ...
                        const dataContentKey = vervarVal + varVal + turvarVal + tahunVal + turtahunVal;
                        const dataContentValue = datacontent[dataContentKey]
                        // 0 means no turtahun
                        const newDataContentKey= turtahun[0].val === 0 ? tahun[thCount].label :  turtahun[turthCount].label + " " + tahun[thCount].label
