    ...
    return (
        <div className="h-full flex-1">
            <div className="text-xs flex items-center justify-between mb-4">
                ...
                <div>Showing {pageAndCount.per_page} out of {pageAndCount.total} </div>
            </div>
            <div className="overflow-auto flex flex-col gap-2 max-h-[70vh] md:max-h-full">
                {dynamicData.map((dynData) => <Link key={dynData.var_id} href={`/bps/var/${dynData.var_id}`} className="hover:bg-blue-100 p-2 rounded-md">
