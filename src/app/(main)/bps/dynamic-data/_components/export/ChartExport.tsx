...
const DynamicDataChartExport = ({chart, dynamicData, datacontent, elementRef}: TDynamicDataChartExport) => {
    return (
    <div className="bg-foreground p-2 w-screen h-screen fixed top-0 right-0 -z-50" ref={elementRef}>
        <div className="mb-4">{dynamicData.var[0].label}</div>
        <div
            className={cn("w-full mb-4", !chart || chart === EDynamicDataChart.Table ? "h-fit" : "h-full max-h-96 max-w-[800px]")}>
