...
const BpsDynamicDataSearchDomainFilter = async ({domainParam}: TBpsDynamicDataSearchDomainFilter) => {
    const domains = await fetchBpsDomainList("all") as TBpsResponse<TBpsDomain[]>;
    ...
    return <div>
        ...
        <Select defaultValue={domainParam ? domainParam : domains.data[1][0].domain_id} name="domains">
            <SelectTrigger>
                <SelectValue/>
            </SelectTrigger>
            <SelectContent>
                {domains.data[1].map((domainItem: TBpsDomain) => <SelectItem
                    value={domainItem.domain_id}
