...
const BpsDynamicDataPage = async ({searchParams}: TBpsDynamicDataPage) => {
    const baseDynamicData = await fetchBpsDynamicData({domain: searchParams.domain, var: searchParams.var});
    const dynamicData = await fetchBpsDynamicData(searchParams);
    const dataIsAvailable = baseDynamicData["data-availability"] === "available" || dynamicData["data-availability"] === "available"
    const datacontent = dataIsAvailable ? processBpsDynamicDataContent(dynamicData) : []
